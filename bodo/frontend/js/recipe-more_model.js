var model = (function(){
   ...
   //  model.getRecipesRemote = function(searchQuery){
   //      doAjax("GET", "/api/home/search/remote?q=" + searchQuery + "/", null, true, function(err, data){
   //          if (err) console.error(err);
   //          else {
   //              //after populating the db, redirect to another page
   ...
   //  };
    ...
    model.getRecipeSummary = function(){
        doAjax("GET", "/search/remote/summary", null, true, function(err, data){
            if (err) console.error(err);
            else {
                console.log(data);
                document.dispatchEvent(new CustomEvent("buildMoreSearchRecipes", {'detail': data}));
            }
        });
    };
